  ...
  switch (method) {
    case "POST": {
      ...
      if (user?.role === "org-admin") {
        ...
        let slug = baseSlug;
        let count = 1;
        while (eventsList.some((event) => event.slug === slug)) {
          slug = `${baseSlug}-${count}`;
          count++;
        }
        ...
        while (!isSlugIdUnique) {
          ...
          isSlugIdUnique = !eventsList.some((event) => event.slugId === slugId);
        }
        ...
        try {
          const product = await db.collection("events").insertOne(updatedData);
