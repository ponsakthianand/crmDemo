...
const mfHandler = async (req, { params }) => {
  const { method } = req;
  ...
  switch (method) {
    case 'POST': {
      ...
      const findCustomer = await dbRxtn.collection('customers').findOne({ _id: customerObjectId });
      if (!findCustomer) {
      ...
      for (const req of requests) {
        const { fundType, transactionType, amount } = req;
    ...
    case 'GET': {
      ...
      try {
        const mutualFunds = await collection.find({ customer_id: customerId }).sort({ created_at: -1 }).toArray();
