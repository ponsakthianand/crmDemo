...
const todoHandler = async (req, { params }) => {
  const { method } = req;
  ...
  switch (method) {
    case 'GET':
      return getTaskHandler(customerId, collection, user);
    case 'POST':
      const body = await req.json();
      return addTaskHandler(body, customerId, db, user);
    case 'PUT':
...
async function getTaskHandler(customerId, collection, user) {
  ...
  try {
    if (user?.role === 'org-admin') {
      const notes = await collection.find({ customer_id: customerId }).sort({ isCompleted: 1, dueDate: 1 }).toArray();
...
async function addTaskHandler(body, customerId, db, user) {
  const { title, dueDate, assign, description } = body;
...
async function editTaskHandler(body, collection, user) {
  const { todoId, title, dueDate, assign, description } = body;
