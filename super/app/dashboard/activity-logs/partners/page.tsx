...
import { fetchPartnerLogsDataAPI } from '@/app/store/reducers/partnersLogs';
import PageContainer from '@/components/layout/page-container';

export default function AdminsTab() {
  const dispatch = useAppDispatch()
  const accessToken = useAppSelector((state) => state.authToken);
  const adminLogs = useAppSelector((state) => state.partnersLogs);
  const adminLogsList = adminLogs?.data;

  useEffect(() => {
    accessToken?.access_token?.length && dispatch(fetchPartnerLogsDataAPI(accessToken?.access_token));
  }, [accessToken])
