...
export function ProductEventForm({ event, token }: EventFormProps) {
  ...
  useEffect(() => {
    if (event) {
      ...
      setTags(event?.tags?.join(', ')); // Convert array to comma-separated string
      setHtml(event?.description);
    }
  }, [event]);
  ...
  const onHandleSave = async () => {
    ...
    const eventData = {
      ...
      tags: cleanedTags,
      description: html,
      image: blob ? [blob?.url] : event?.image?.length ? event?.image : [],
    };

    try {
      const response = await fetch(event ? `/api/events/${event._id}` : '/api/events', {
    ...
    <div>
      <div className='h-full flex-1 flex-col px-8 md:flex'>
        <div className='flex items-center justify-between space-y-2'>
          <div>
            <h2 className='text-2xl font-bold tracking-tight'>{event ? 'Edit Event' : 'New Event'}</h2>
          </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-10 gap-4 mt-4 min-h-screen auto-rows-fr">
              ...
              <div className='w-full mt-6'>
                ...
                <div className="flex rounded-lg shadow-sm relative">
                  <div className="flex items-center justify-center pointer-events-none text-gray-500 text-sm px-3 pr-0 py-1 rounded rounded-r-0 h-9 dark:text-neutral-500 border-[1px] border-r-0 bg-gray-50">
                    <span>https://www.rxtn.in/sessions/</span>
                  </div>
                  ...
                  />
                  {formData.slug ?
                    (<Link href={`https://www.rxtn.in/sessions/${formData.slug}`} target="blank" className="p-1 px-2 border bg-gray-50 rounded-r-lg hover:bg-blue-700 group">
              ...
              <div className="grid grid-cols-1 gap-4 lg:grid-cols-4">
                <div className="w-full mt-6">
                  ...
                  <Input className="bg-white"
                    type={'text'}
                    name={'regularPrice'}
                    placeholder={'Event Regular Price'}
          ...
          <div className="lg:col-span-3 h-[600px] flex flex-col">
            <div className="p-4 border rounded shadow bg-gray-50 flex-1">
              ...
              <div className="w-full mt-6">
                {(blob || event?.image?.length) ? (
                  <div className="mt-4">
                    <Label htmlFor={''}>
                      Selected Image
                    </Label>
                    <div className="relative">
                      <Image alt="" src={blob?.url ? blob?.url : event?.image?.length ? event?.image[0] : ''} width={100} height={100} className="text-blue-500 underline" />
                    </div>
                  </div>
                ) : <></>}
              </div>
