...
import { ChevronDown, ChevronRight, Check, Info, Trash2, WandSparkles, CalendarDays, Loader2 } from "lucide-react"
...
import { EditMutualFundDialog } from "../customers/[customerId]/mutual-funds/edit-mf"
import { CustomersData } from "@/app/store/reducers/allCutomers"
import { FundStatusBadge } from "./utils/mfUtils";
import TableLoader from "@/components/ui/table-loader";

interface FinancialTrackerTabProps {
  mutualFunds: MutualFundsDocument[];
  customers: CustomersData[];
  onSubmit: () => void;
  loading: boolean;
}

export default function MutualFundsTab({ mutualFunds, customers, onSubmit, loading }: FinancialTrackerTabProps) {
  const dispatch = useAppDispatch()
  const accessToken = useAppSelector((state) => state.authToken);
  const mfList = mutualFunds;
  ...
  const onDelete = async (id: string, customerID: string) => {
  ...
  const sendMagicLink = async (id: string, customerID: string) => {
  ...
  const onApprove = async (id: string, customerID: string) => {
    const response = await fetch(`/api/customers/${customerID}/mf/${id}`, {
      method: "PUT",
      headers: {
        "Content-Type": "application/json",
  ...
  const deleteCall = (id: string, status: string, customerID: string) => {
    return (
      <Dialog>
        ...
        <DialogContent className="sm:max-w-[425px]">
          ...
          <DialogFooter>
            ...
            <DialogClose asChild>
              <Button type="button" onClick={() => onDelete(id, customerID)}>Delete</Button>
            </DialogClose>
          </DialogFooter>
        </DialogContent>
      </Dialog>
  ...
  const approveCall = (id: string, status: string, mf: any, customerID: string) => {
    return (
      status !== 'Approved' ? <>
        <div className='hover:text-blue-500'>
          <EditMutualFundDialog customerId={customerID} token={accessToken?.access_token || ''} mfRequest={mf} customers={customers} onSuccess={onSubmit} />
        </div>
        ...
        <Dialog>
          ...
          <DialogContent className="sm:max-w-[425px]">
            ...
            <DialogFooter>
              ...
              <DialogClose asChild>
                <Button type="button" onClick={() => onApprove(id, customerID)}>Approve</Button>
              </DialogClose>
            </DialogFooter>
          </DialogContent>
        </Dialog>
  ...
  return (
    <div className="rounded-md border">
      <Table>
        ...
        <TableBody>
          {!loading ? mfList?.length ? mfList.map((fund, index) => (
            <TableRow key={fund._id}>
              <TableCell>{format(new Date(fund.created_at), "PPpp")}</TableCell>
              ...
              <TableCell>
                <div className="flex space-x-2">
                  ...
                  {fund?.status !== 'Approved' ? (<Button type="button" variant="default" size="sm" onClick={() => sendMagicLink(fund._id, fund.customer_id)}>
                    {isSubmitting.id === fund._id && isSubmitting.status ? (
                      <>
                  ...
                  {approveCall(fund._id, fund.status, fund, fund.customer_id)}
                  {deleteCall(fund._id, fund.status, fund.customer_id)}
                </div>
              </TableCell>
            </TableRow>
