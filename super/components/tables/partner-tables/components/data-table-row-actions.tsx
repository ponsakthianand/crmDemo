...
export function DataTableRowActions<TData>({
  ...
  const onDelete = async (leadId: string, name: string) => {
    const response = await fetch(`/api/delete/partners/${leadId}`, {
      method: "DELETE",
      headers: {
    ...
    toast({
      title: "Deleted",
      description: `${name} - partner has been deleted successfully.`,
      variant: "destructive",
    })
    const data = await response.json();
    if (accessToken?.access_token) {
      dispatch(fetchPartnersDataAPI(accessToken.access_token));
    }
    return data;
  }
