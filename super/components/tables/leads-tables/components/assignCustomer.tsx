...
export function AssignCustomer({ kid, customer_id }: any) {
  ...
  const updateTicket = async (partner_user_id: any) => {
    await fetch(`${baseUrl}/customer/assign`, {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        token: `Bearer ${accessToken?.access_token}`,
      },
      body: JSON.stringify({ partner_user_id, customer_id })
    })
      .then(async (response) => {
        if (response.ok) {
