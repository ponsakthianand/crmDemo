...
import { allCourseAction } from '@/app/store/reducers/allCourses';
import { Trash2, FilePenLine, ExternalLink } from 'lucide-react';

const EditButton = ({ row, icon }) => {
  const router = useRouter();
  const onHandleEditCourse = () => {
    router.push(`/dashboard/courses/${row.original._id}`);
  };
  return (
    <div onClick={onHandleEditCourse}>
      {icon === 'edit' ? (
        <FilePenLine
...
const DeleteButton = ({ row, icon }) => {
  ...
  const onHandleDeleteCourse = () => {
    dispatch(
      allCourseAction.deleteCourse({
        open: true,
  ...
  return (
    <div onClick={onHandleDeleteCourse}>
  ...
  },
  {
    ...
    cell: ({ row }) => (
      <div className='flex'>
        <Link
          target='_blank'
          href={`https://www.rxtn.in/courses/${row.original.slug}`}
        >
