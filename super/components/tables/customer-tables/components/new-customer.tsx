...
export default function NewCustomerDialog() {
  ...
  const partner = partnersList?.data?.find(user => user?._id === value)
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    await fetch(`/api/customers/register`, {
      ...
      body: JSON.stringify({ email, name, phone, partner_user_id: partner?.partner_user_id, partner_id: partner?._id, partner_name: partner?.name, partner_phone: partner?.phone, })
    })
      .then(async (response) => {
        setLoading(false)
        setValue('')
