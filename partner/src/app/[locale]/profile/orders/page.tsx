...
import { fetchOrdersDataAPI } from '@/src/app/store/reducers/allOrders';
...
export default function Orders() {
  ...
  useEffect(() => {
    accessToken?.access_token?.length && dispatch(fetchOrdersDataAPI(accessToken?.access_token, null));
  }, [accessToken])
          ...
          <div className="relative">
            <div className="w-full md:w-12/12 min-h-[550px]">
              {!ordersList?.loading && orders?.length ? (
                ...
                !ordersList?.loading ? <div className='p-5'>You have not referred any orders yet</div> : <div className='p-5'><TableLoader /></div>
              )}
            </div>
          </div>
        </div>
