...
import UsernameUpdater from '@/src/app/common/elements/usernameUpdate';

const ProfileEditPage = () => {
  const [loader, setLoader] = useState(false);
  const accessToken = useAppSelector((state) => state.authToken);
  const getCustomerInfo = useAppSelector((state) => state.profileData);
  let currentUser = getCustomerInfo?.data;
  ...
  useEffect(() => {
    ...
    setConfidentialData(confidentialDataMapped)
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    let payload = { role: 'partner', created_at: new Date() };
    ...
    if (response.ok) {
      toast.success('Changes succesfully updated!')
      setLoader(false)
    } else {
          ...
          <div className={`${loader ? 'opacity-20 pointer-events-none	' : ''}`}>
            <form className="w-full" method="POST" onSubmit={(event) => handleSubmit(event)}>
              ...
              <div className='flex justify-end'>
                <button type="submit" disabled={disabled} className="text-white bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-5 py-2.5 text-center dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800 disabled:bg-slate-400">
                  Save
                </button>
              </div>
