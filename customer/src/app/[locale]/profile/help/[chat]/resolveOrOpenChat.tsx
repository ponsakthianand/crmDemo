...
import { fetchCurrentTicketDataAPI } from '@/src/app/store/reducers/currentTicket';

const options = [
  { value: 'solved', label: 'Got the solution' },
  { value: 'foundSomewhere', label: 'Found solution Somewhere' },
  { value: 'noLongerNeeded', label: 'No longer needed' },
  { value: 'other', label: 'Other' },
]

export default function OpenCloseChat({ ticketId }) {
  ...
  const handleClose = async (event: FormEvent<HTMLFormElement>) => {
    ...
    const response = await fetch(`${baseUrl}/tickets/${ticketId}/close`, {
      method: 'POST',
      headers: {
    ...
    if (response.ok) {
      ...
      setDescription('')
      close();
      accessToken?.access_token?.length && dispatch(fetchCurrentTicketDataAPI(ticketId, accessToken?.access_token));
      accessToken?.access_token?.length && dispatch(fetchTicketsDataAPI(accessToken?.access_token));
      toast('Your ticket successfully closed')
      setLoader(false)
    } else {
      accessToken?.access_token?.length && dispatch(fetchTicketsDataAPI(accessToken?.access_token));
  ...
  return (
    <>
      <div className="py-2 px-3 cursor-pointer inline-flex items-center gap-x-2 text-sm font-semibold rounded-lg border border-transparent bg-blue-600 text-white hover:bg-[#008755]"
        onClick={open}>
        <PiCheckFatDuotone />
      </div>
          ...
          <div className="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
            ...
            >
              <div className={`sm:mx-auto sm:w-full items-center ${loader ? 'opacity-20 pointer-events-none	' : ''}`}>
                <form className="" action="#" method="POST" onSubmit={(event) => handleClose(event)}>
                      ...
                      <div className="mt-3 text-center sm:mt-0 sm:text-left">
                        ...
                        <div className="mt-2">
                          <div className='mt-5'>
                            ...
                            <div className="mt-2">
                              <Select name='title' options={options} onChange={(val) => setOption(val.value)} />
                            </div>
                          </div>
                          {
                            option === 'other' ? <div className='mt-5'>
                              <div className="flex items-center justify-between">
                                <label htmlFor="description" className="block text-sm font-medium leading-6 text-gray-900">
                                  Please specify
                                </label>
                  ...
                  <div className="bg-gray-50 mt-0 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
                    <button type="submit" disabled={!option?.length}
                      className="inline-flex w-full justify-center rounded-md bg-blue-700 hover:bg-blue-800 px-3 py-2 text-sm font-semibold text-white shadow-sm sm:ml-3 sm:w-auto disabled:opacity-50 disabled:pointer-events-none"
                    >
                      Resolve
                    </button>
                    <button
